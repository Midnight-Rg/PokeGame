...
Pokémon sağlığı: {self.hp}"""
    ...
    async def attack(self, enemy):
        ...
        if enemy.hp > self.power:
            enemy.hp -= self.power
            return f"Pokémon eğitmeni @{self.pokemon_trainer}, @{enemy.pokemon_trainer}'ye saldırdı.\n@{enemy.pokemon_trainer}'nin sağlık durumu şimdi {enemy.hp}"
        else:
            enemy.hp = 0
            return f"Pokémon eğitmeni @{self.pokemon_trainer}, @{enemy.pokemon_trainer}'yi yendi!"


class Wizard(Pokemon):
    # Sihirbaz Pokémon’a özel şeyler buraya eklenebilir
    pass
